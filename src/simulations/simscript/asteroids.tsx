import {
    SimulationState, Simulation, Entity, Queue, Uniform,
    IPoint, Point, Event, EventArgs, setOptions
} from 'simscript';
import { SimulationComponent, BooleanParameter } from '../../simscript-react/components';

/**
 * Custom Component to show Asteroids Simulations with 
 * parameters and an SVG animation.
 */
export class AsteroidsComponent extends SimulationComponent<Asteroids> {

    // render parameters section
    renderParams(): JSX.Element {
        const sim = this.props.sim;
        return <div style={{ display: 'flex', justifyContent: 'space-around' }}>
            <BooleanParameter label='Professional Mode' parent={this} value={sim.proMode} change={v => sim.proMode = v} />
            <label>
                Missiles Fired: <b>{sim.missilesFired}</b>
            </label>
            <label>
                Asteroids Destroyed: <b>{sim.asteroidsDestroyed}</b> / {sim.asteroidCount}
            </label>
        </div>;
    }
    ...
    getAnimationHostHtml(): string {
        return `<svg class='ss-anim asteroids' viewBox='0 0 1000 500'
        ...
        return {
            ...
            getEntityHtml: (e: Entity) => {
                if (e instanceof Ship) {
                    return `<g>
                    <polygon fill='none' stroke='white' stroke-width='2'
                        points='0 0, -20 30, 40 0, -20 -30'/>
                    <circle r='25' cx='-30' fill='url(#jetGradient)'
                        opacity="${e.engineOn ? '1' : '0'}"/>
                </g>`;
                } else if (e instanceof Missile) {
                    return `<g>
                        <line x1='-10' y1='0' x2='10' y2='0' stroke='red' stroke-width='6'/>
                    </g>`;
    ...
    initializeAnimation(animHost: HTMLElement): void {
        const
            sim = this.props.sim,
            gameOver = animHost.querySelector('.game-over') as SVGTextElement;
...
export class Asteroids extends Simulation {
    ...
    // gets or sets the animation speed (pro mode is faster).
    get proMode(): boolean {
        return this.frameDelay === PRO_DELAY;
    }
    set proMode(value: boolean) {
        this.frameDelay = value ? PRO_DELAY : DEF_DELAY;
    }

    // game over!
    gameOver(won: boolean) {
        this.won = won;
        this.q.entities.forEach(e => (e as Flyer).done = true);
        setTimeout(() => {
    ...
    _touchend(e: TouchEvent) {
        ...
        if (svg && this._touch) {
            ...
            (this.ship as Ship).keydown(new KeyboardEvent('keydown', { key: key, shiftKey: true }));
        }
        this._touch = null;
    }
}
